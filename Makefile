...
.PHONY: build status run run-more clean compose-up compose-down logs monitor example-no-mount example-mount example-volume example-down net-no-network net-ports net-network net-down break-stage1 fix-stage1 break-stage2 fix-stage2 break-stage3 fix-stage3 break-stage4 fix-stage4 chaos-list clean-all help
.DEFAULT_GOAL := help
...
# Самостоятельные задания: compose-стеки и контейнеры
EXEC_COMPOSE_DIRS := steps/step_2_control/execution steps/step_3_mount/execution steps/step_4_network/execution
EXEC_CONTAINERS   := c-orders-frontend c-backend-orders-api c-frontend-orders

# Все образы лабы — для отлова авто-именованных контейнеров (docker run --rm без --name)
LAB_IMAGES := docker-lab/orders-api docker-lab/orders-frontend docker-lab/orders-api-broken mount-example orders-frontend
...
status: ## Показать образы и запущенные контейнеры
	...
	@echo ""
	@echo "$(YELLOW)  Колонки:$(NC)"
	@echo "$(YELLOW)   DRIVER        — драйвер вольюма (обычно local)$(NC)"
	@echo "$(YELLOW)   VOLUME NAME   — имя вольюма$(NC)"
	@echo ""
	...
	@echo ""
	@echo "$(YELLOW)  Колонки:$(NC)"
	@echo "$(YELLOW)   NETWORK ID    — уникальный идентификатор сети$(NC)"
	@echo "$(YELLOW)   NAME          — имя сети$(NC)"
	@echo "$(YELLOW)   DRIVER        — драйвер (bridge / host / none / overlay)$(NC)"
	@echo "$(YELLOW)   SCOPE         — область действия (local / swarm)$(NC)"
	@echo ""
...
run: ## Запустить контейнер на порту 8081
	...
	@echo "$(YELLOW)(или http://<IP-вашей-ВМ>:$(BASE_PORT), если вы работаете на виртуальной машине)$(NC)"
...
run-more: ## Запустить ещё один контейнер (авто-выбор порта)
	...
	echo "$(YELLOW)(или http://<IP-вашей-ВМ>:$$PORT, если вы работаете на виртуальной машине)$(NC)"; \
	echo ""; \
...
compose-up: ## Поднять стек через docker compose
	...
	@echo "$(YELLOW)(или http://<IP-вашей-ВМ>:$(BACKEND_PORT_2), если вы работаете на виртуальной машине)$(NC)"
	@echo ""
...
clean-all: ## Остановить и удалить все контейнеры лабы (лекция + самостоятельные). Образы не трогаем
	@echo "$(YELLOW)════════════════════════════════════════════════════════$(NC)"
	@echo "$(YELLOW)Полная уборка контейнеров: лекция + самостоятельные + chaos$(NC)"
	@echo "$(YELLOW)(образы остаются на месте)$(NC)"
	@echo "$(YELLOW)════════════════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(YELLOW)[1/3] Compose-стеки лекции и chaos$(NC)"
	$(chaos_cleanup)
	@echo "$(YELLOW)[2/3] Compose-стеки самостоятельных заданий$(NC)"
	@for d in $(EXEC_COMPOSE_DIRS); do \
		if [ -f $(CURDIR)/$$d/docker-compose.yml ]; then \
			echo "$(GREEN)  ▶  docker compose down   ($$d)$(NC)"; \
			(cd $(CURDIR)/$$d && docker compose down 2>/dev/null) || true; \
		fi; \
	done
	@echo ""
	@echo "$(YELLOW)[3/3] Оставшиеся контейнеры по имени и по образу$(NC)"
	@for c in $(NET_BACK_CONT) $(NET_FRONT_CONT) $(EXAMPLE_MOUNT_CONT) $(CHAOS_STAGE1_CONT) $(EXEC_CONTAINERS); do \
		if docker ps -a --format '{{.Names}}' | grep -qx "$$c"; then \
			echo "$(GREEN)  ▶  docker rm -f $$c$(NC)"; \
			docker rm -f $$c >/dev/null 2>&1 || true; \
		fi; \
	done
	@for cid in $$(docker ps -aq); do \
		orig=$$(docker inspect --format '{{.Config.Image}}' $$cid 2>/dev/null); \
		base=$$(echo "$$orig" | sed 's/:.*//'); \
		for lab in $(LAB_IMAGES); do \
			if [ "$$base" = "$$lab" ]; then \
				echo "$(GREEN)  ▶  docker rm -f $$cid   ($$orig)$(NC)"; \
	...
	@echo ""
	@echo "$(YELLOW)Готово. Контейнеры лабы убраны, образы сохранены.$(NC)"
...
help: ## Показать список команд
	...
	@echo ""
	@echo "$(YELLOW)   Тренировка: сломанные сценарии$(NC)"
	@echo "$(YELLOW)   make chaos-list         Список доступных сценариев$(NC)"
	@echo "$(YELLOW)   make break-stageN       Запустить сломанный сценарий (N = 1..4)$(NC)"
	@echo "$(YELLOW)   make fix-stageN         Разбор причины + уборка (N = 1..4)$(NC)"
	@echo ""
	@echo "$(YELLOW)   Полная уборка$(NC)"
	@echo "$(YELLOW)   make clean-all          Снести все контейнеры: лекция + самостоятельные + chaos$(NC)"
	@echo "$(YELLOW)════════════════════════════════════════════════════════$(NC)"
